...
def login():
    ...
    date_expiry = date(2022, 10, 12)
    date_current = date.today()

    if (date_current <= date_expiry) and ((e == 'C142810') or (e == 'STAC1082')) and ((f == '38125477606') or (f == '33200124763')):
    ...
    if len(TRADED_SYMBOL) == 0 :
        ...
        try:
            ...
            if len(df_nifty) != None :
                ...
                if (df_nifty['CROSS_UP'].iloc[-1] == 1 or df_nifty['CROSS_UP'].iloc[-2] == 1) and nifty_ta() == 1:
...
if __name__ == '__main__':
    ...
    obj=SmartConnect(api_key=cred.apikey)
    data = obj.generateSession(cred.username,cred.pwd,totp.now())
    session_key = input('Enter session key = ')
    breeze = BreezeConnect(api_key=cred.api_key_icici) 
    breeze.generate_session(api_secret=cred.secret_key_icici,session_token=session_key)
